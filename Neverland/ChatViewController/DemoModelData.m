...
- (instancetype)initWithRoomNumber:(NSString *)numberString withGroupName: (NSString *)groupName withCharacterImage: (UIImage *)characterImage withcharacterImageName:(NSString *)characterImageName {
    ...
    if (self) {
        ...
        self.roomReferenceHandle = [self.roomReference observeEventType:FIRDataEventTypeChildAdded withBlock:^(FIRDataSnapshot * _Nonnull snapshot) {
            NSDictionary *obj = snapshot.value;
            NSLog(@"snapshot has been updated %@", obj);
            
            if ([obj isKindOfClass:[NSDictionary class]]) {
                NSDictionary *currentMessageDict = obj;
                if ([currentMessageDict isKindOfClass:[NSDictionary class]] && ![[currentMessageDict valueForKey:@"senderId"] isEqualToString:self.senderId]) {
                    NSLog(@"we got a new external message");
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"ReceivedNewMessage" object:self userInfo:currentMessageDict];
                }
            }
        }];
...
- (instancetype)init
{
    return [self initWithRoomNumber:@"0" withGroupName:@"Popular" withCharacterImage:[UIImage imageNamed:@"Donald Trump 1"] withcharacterImageName:@"Donald Trump 1"];
}
